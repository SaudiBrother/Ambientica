...
class DAWApp {
    ...
    initAudioGate() {
        if (this.audio.ctx.state === 'running') { this.hideOverlay(); return; }

        this.dom.overlayText.textContent = 'Ambientica';
        this.dom.overlaySub.textContent = 'Tap or click anywhere to power on the audio engine';
        this.dom.overlayLayer.classList.add('ready', 'active');

        const start = async () => {
            try { await this.audio.ctx.resume(); } catch (e) { /* ignore */ }
