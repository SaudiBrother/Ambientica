...
class DAWApp {
    constructor() {
        ...
        this.sliders = {}; // sliders[fxId][paramId] = { slider, updateVal } — populated by createSlider()
        this._applyingPreset = false; // true while applyPreset()/restoring Custom is running
    ...
    init() {
        window.__app = this; // handy for debugging from devtools
        this.cacheDOM();
        this.initState();
    ...
    cacheDOM() {
        ...
        this.dom = {
            ...
            audioPresetSelector: $('#audio-preset-selector'),
            helpBtn: $('#help-btn'), helpPage: $('#help-page'),
            helpCloseBtn: $('#help-close-btn'), helpContent: $('#help-content'),
    ...
    initState() {
        ...
        // Fresh install (or after "Reset Semua"): start from the default
        // preset's curated values rather than each control's raw factory
        // default — those single-knob defaults stay reachable via each
        // slider's own double-click-to-reset (see createSlider's
        // `defaultValue: conf.value`).
        if (!persisted && typeof AUDIO_PRESETS !== 'undefined' && AUDIO_PRESETS[DEFAULT_PRESET]) {
            const def = AUDIO_PRESETS[DEFAULT_PRESET];
            for (const fxId in (def.fx || {})) {
                if (!this.state.fxParams[fxId]) continue;
                for (const paramId in def.fx[fxId]) {
                    if (paramId in this.state.fxParams[fxId]) {
                        this.state.fxParams[fxId][paramId] = def.fx[fxId][paramId];
                    }
                }
            }
            if (typeof def.masterVolume === 'number') {
                this.state.masterVolume = Math.max(0, Math.min(1.25, def.masterVolume));
                this.state.lastVolume = this.state.masterVolume > 0 ? this.state.masterVolume : 1;
            }
        }

        // Audio Presets: brand-new installs start on the default preset
        // (its values already match the EFFECTS_CONFIG defaults above, so
        // nothing else needs to change). Returning users without this field
        // in their saved data land on "Custom" so migration never silently
        // re-labels their existing settings as a named preset.
        const validPresetKeys = new Set([...PRESET_ORDER, 'custom']);
        this.state.activePreset = (persisted && typeof persisted.activePreset === 'string' && validPresetKeys.has(persisted.activePreset))
            ? persisted.activePreset
            : (persisted ? 'custom' : DEFAULT_PRESET);
        this.state.customSnapshot = (persisted && persisted.customSnapshot) ? persisted.customSnapshot : null;
    }
    ...
    _buildStateSnapshot() {
        return {
            ...
            customSnapshot: this.state.customSnapshot
        };
    }
    ...
    initMasterVolume() {
        this.masterVolumeSlider = new RangeSlider(this.dom.masterVolumeSlot, {
            ...
            defaultValue: 1,
            ariaLabel: 'Master volume (Gain)',
            onChange: (v) => { this.setMasterVolume(v); this.maybeSwitchToCustomPreset(); }
        });
    ...
    /** Wires the "Audio Presets" dropdown. The sliders/nodes for the
     * current fxParams + masterVolume already exist by the time this
     * runs (called after renderFXChain()), so nothing audible changes
     * here — this only syncs the dropdown to state.activePreset and
     * reacts when the user picks a different option. */
    initAudioPresets() {
        const sel = this.dom.audioPresetSelector;
        if (!sel) return;
        sel.value = this.state.activePreset;

        sel.addEventListener('change', (e) => {
            const key = e.target.value;

            if (key === 'custom') {
                if (this.state.customSnapshot) this._applyPresetValues(this.state.customSnapshot);
            ...
            // Leaving "Custom" for a named preset: remember the current
            // values first, so picking "Custom" again later restores them.
            if (this.state.activePreset === 'custom') {
                this.state.customSnapshot = this._takeFullSnapshot();
            }
            this.applyPreset(key);
        });
    }

    /** Applies every value defined by a named preset (js/presets-config.js)
     * to state, audio nodes, sliders and their readouts, and marks it as
     * the active preset. */
    applyPreset(key, persist = true) {
        const preset = AUDIO_PRESETS[key];
        if (!preset) return;
        this._applyPresetValues(preset);
        this.state.activePreset = key;
        if (this.dom.audioPresetSelector) this.dom.audioPresetSelector.value = key;
        if (persist) this.saveState();
    }

    /** Shared by applyPreset() and "restore Custom": pushes a
     * { masterVolume, fx } snapshot into state + audio nodes + sliders
     * without it counting as a user edit (see maybeSwitchToCustomPreset). */
    _applyPresetValues(values) {
        this._applyingPreset = true;
        try {
            if (typeof values.masterVolume === 'number') {
                this.setMasterVolume(values.masterVolume, false);
            }
            for (const fxId in (values.fx || {})) {
                if (!this.state.fxParams[fxId]) continue;
                const params = values.fx[fxId];
                for (const paramId in params) {
                    const v = params[paramId];
                    this.state.fxParams[fxId][paramId] = v;
                    this.updateNodeParam(fxId, paramId, v);
                    const entry = this.sliders[fxId] && this.sliders[fxId][paramId];
                    if (entry) {
                        entry.slider.setValue(v, false);
                        entry.updateVal(entry.slider.value);
                    }
                }
            }
        } finally {
    ...
    /** Captures every effect's current numeric params + master volume —
     * everything a preset can define — as a plain object for
     * state.customSnapshot. Bypass flags are intentionally excluded:
     * presets only ever touch continuous parameters. */
    _takeFullSnapshot() {
        const fx = {};
        for (const fxId in this.state.fxParams) {
            const src = this.state.fxParams[fxId];
            const copy = {};
            for (const paramId in src) {
                if (paramId === 'bypass') continue;
                copy[paramId] = src[paramId];
            }
            fx[fxId] = copy;
        }
        return { masterVolume: this.state.masterVolume, fx };
    }

    /** Called after any manual change to an effect parameter, an
     * effect's bypass switch, or the master volume. If a named preset is
     * currently selected, editing anything detaches it to "Custom" — the
     * just-made edit becomes (the start of) the new Custom state. */
    maybeSwitchToCustomPreset() {
        if (this._applyingPreset) return;
        if (this.state.activePreset === 'custom') return;
        this.state.activePreset = 'custom';
        if (this.dom.audioPresetSelector) this.dom.audioPresetSelector.value = 'custom';
    }
    ...
    initHelpPage() {
        const open = () => {
            this.dom.helpPage.classList.add('active');
            document.body.classList.add('help-open');
        };
        const close = () => {
            this.dom.helpPage.classList.remove('active');
            document.body.classList.remove('help-open');
        };
        this.dom.helpBtn.addEventListener('click', open);
        this.dom.helpCloseBtn.addEventListener('click', close);
        document.addEventListener('keydown', (e) => {
            if (e.key === 'Escape' && this.dom.helpPage.classList.contains('active')) close();
        });
    }

    /** Builds the whole #help-content from HELP_GENERAL_SECTIONS (general
     * features), AUDIO_PRESETS (preset descriptions), EFFECTS_CONFIG +
     * HELP_EFFECT_DETAILS (one section per effect, reusing each effect's
     * own icon/color from the rack) and HELP_SHORTCUTS. Runs once at
     * startup — the page is just shown/hidden afterwards. */
    renderHelpPage() {
        const root = this.dom.helpContent;
        if (!root || typeof HELP_GENERAL_SECTIONS === 'undefined') return;
        root.innerHTML = '';

        const makeSection = (icon, title, bodyEl, color) => {
            const sec = document.createElement('div');
            sec.className = 'help-section';
            const iconEl = document.createElement('div');
            iconEl.className = 'help-section-icon';
            ...
            body.append(h3, bodyEl);
            sec.append(iconEl, body);
            return sec;
        };

        // 1. General, non-effect sections (Theme, Audio Presets, IO, etc.)
        HELP_GENERAL_SECTIONS.forEach(s => {
            const wrap = document.createElement('div');
            const p = document.createElement('p');
            p.textContent = s.body;
            wrap.appendChild(p);

            if (s.id === 'presets' && typeof AUDIO_PRESETS !== 'undefined') {
                const list = document.createElement('ul');
                list.className = 'help-preset-list';
                PRESET_ORDER.forEach(key => {
                    const preset = AUDIO_PRESETS[key];
                    if (!preset) return;
                    const li = document.createElement('li');
                    li.className = 'help-preset-item';
                    const strong = document.createElement('strong');
                    strong.textContent = preset.label + ': ';
                    li.append(strong, document.createTextNode(preset.description));
                    list.appendChild(li);
                });
                const customLi = document.createElement('li');
                customLi.className = 'help-preset-item';
                const customStrong = document.createElement('strong');
                customStrong.textContent = 'Custom: ';
                customLi.append(customStrong, document.createTextNode(
                    typeof CUSTOM_PRESET_DESCRIPTION !== 'undefined' ? CUSTOM_PRESET_DESCRIPTION : ''
                ));
                list.appendChild(customLi);
                wrap.appendChild(list);
            }

            if (s.id === 'shortcuts' && typeof HELP_SHORTCUTS !== 'undefined') {
                const list = document.createElement('div');
                list.className = 'help-shortcut-list';
                HELP_SHORTCUTS.forEach(({ keys, desc }) => {
                    const row = document.createElement('div');
                    row.className = 'help-shortcut-row';
                    const kbd = document.createElement('kbd');
                    kbd.textContent = keys;
                    row.append(kbd, document.createTextNode(desc));
                    list.appendChild(row);
                });
                wrap.appendChild(list);
            }

            root.appendChild(makeSection(s.icon, s.title, wrap));
        });

        // 2. One section per effect in the rack, reusing its own icon/color.
        const groupTitle = document.createElement('div');
        groupTitle.className = 'help-page-group-title';
        groupTitle.textContent = 'Efek di Rantai Efek';
        root.appendChild(groupTitle);

        const order = (typeof DEFAULT_CHAIN_ORDER !== 'undefined') ? DEFAULT_CHAIN_ORDER : Object.keys(EFFECTS_CONFIG);
        order.forEach(fxId => {
            const config = EFFECTS_CONFIG[fxId];
            const details = HELP_EFFECT_DETAILS[fxId];
            ...
            const list = document.createElement('ul');
            list.className = 'help-param-list';
            for (const paramId in config.params) {
                const desc = details.params[paramId];
                if (!desc) continue;
                const li = document.createElement('li');
                const strong = document.createElement('strong');
                strong.textContent = config.params[paramId].name;
                li.append(strong, document.createTextNode(desc));
                list.appendChild(li);
            }
            wrap.appendChild(list);

            root.appendChild(makeSection(config.icon, config.name, wrap, config.color));
        });
    }
    ...
    initEventListeners() {
        ...
        this.dom.downloadBtn.addEventListener('click', async () => {
            ...
            this.dom.downloadBtn.innerHTML = `<i class="fa-solid fa-circle-notch fa-spin"></i> Mengekspor...`;
            await new Promise(r => setTimeout(r, 30));
            await this.handleDownload();
            this.dom.downloadBtn.innerHTML = `<i class="fa-solid fa-file-export"></i> Ekspor WAV`;
        });

        this.dom.playBtn.addEventListener('click', async () => {
            if (this.audio.ctx.state !== 'running') await this.resumeAudioContext();
        ...
        this.dom.resetBtn.addEventListener('click', async () => {
            const ok = await this.showConfirm(
                'Reset semua?',
                'Ini akan mengembalikan setiap efek, tema, preset, volume utama, dan susunan rak ke pengaturan awal. Tindakan ini tidak dapat dibatalkan.'
            );
            if (ok) {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            switch (e.key) {
                ...
                case 'Spacebar':
                    ...
                    e.preventDefault();
                    if (this.audio.ctx.state !== 'running') this.resumeAudioContext();
                    this.state.isPlaying ? this.pause() : this.play();
                    break;
                case 'ArrowRight':
                    e.preventDefault();
    ...
    initDragDropFile() {
        ...
        window.addEventListener('drop', async (e) => {
            ...
            else this.showToast('Silakan seret berkas audio', 'error');
        });
    }

    /** Attempts to resume a suspended AudioContext, but never blocks forever.
     *
     * Some Chrome builds (including Chrome Beta) leave the resume() promise
     * pending indefinitely when the context "is not allowed to start" yet —
     * per the Web Audio spec, that promise is just queued and may never be
     * resolved OR rejected (see WebAudio/web-audio-api#1759, Chromium
     * issue 41413980). An `await` on a promise that never settles freezes
     * whatever called it forever. Racing it against a short timeout
     * guarantees callers always get an answer.
     */
    resumeAudioContext(timeoutMs = 1200) {
        const ctx = this.audio.ctx;
        if (!ctx) return Promise.resolve(false);
        if (ctx.state === 'running') return Promise.resolve(true);

        const resumed = ctx.resume().then(() => true).catch(() => false);
     ...
     *
     * WHY click/touchstart/pointerdown/keydown, AND a direct button listener:
     * iOS Safari and some Android WebViews only accept 'touchstart', 'touchend',
     * and 'click' as valid user-gesture tokens for AudioContext.resume(), while
     * desktop browsers rely on 'click'/'pointerdown'. Covering all of them, plus
     * a listener on the button itself, makes the gate as robust as possible.
     *
     * WHY the overlay is dismissed BEFORE resume() settles:
     * resume() can hang forever on some Chrome builds (see resumeAudioContext
     * above). If hiding the overlay waited on that promise, the overlay would
     * stay stuck on screen forever after the very first tap — exactly the
     * "Tap to Start does nothing" symptom. So the gesture immediately closes
     * the overlay; resuming the audio engine happens in the background and is
     * retried on the next gesture (e.g. pressing Play) if it didn't take.
     */
    initAudioGate() {
        ...
        this.dom.overlaySub.textContent = 'Ketuk atau klik di mana saja untuk mulai';
        this.dom.overlayLayer.classList.add('ready', 'active');

        // Real button inside overlay — most reliable activation on iOS Safari,
        // and gives every browser an explicit, large tap target.
        const tapBtn = document.createElement('button');
        tapBtn.type = 'button';
        tapBtn.className = 'overlay-tap-btn';
        tapBtn.textContent = '\u25b6  Ketuk untuk Mulai';
        ...
        const start = () => {
            ...
            ['click', 'touchstart', 'pointerdown', 'keydown'].forEach(t =>
                document.removeEventListener(t, start));
            tapBtn.removeEventListener('click', start);

            // Dismiss the overlay right away — see note above about resume().
            this.dom.overlayLayer.classList.remove('ready');
            this.hideOverlay();

            // Wake the audio engine in the background (fire-and-forget).
        ...
        tapBtn.addEventListener('click', start);
    }
    ...
    /** Shows the compact Install button (header, next to Bantuan) only
     * when the browser actually offers the PWA install prompt — and
     * keeps it hidden if Ambientica is already running as an installed
     * app (no "install" action makes sense there). */
    initInstallPrompt() {
        ...
        if (isStandalone) return; // stays hidden permanently
        ...
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (outcome === 'accepted') this.showToast('Ambientica berhasil dipasang');
            } finally {
                deferredPrompt = null;
                btn.hidden = true;
                btn.disabled = false;
    ...
    async loadAudioFile(file) {
        if (!file) return;
        await this.triggerOverlay('Mengimpor Audio...', `Mendekode "${file.name}"`, 0);
        ...
        try {
            ...
            this.hideOverlay();
            this.showToast('Audio berhasil dimuat');
        } catch (err) {
            console.error(err);
            this.dom.fileName.textContent = 'Gagal memuat berkas';
            this.hideOverlay();
            this.showToast('Berkas tidak dapat dimuat', 'error');
        }
    }
    ...
    toggleCompare() {
        ...
        this.showToast(this.state.compareMode ? 'Mode Banding — mendengarkan sinyal asli tanpa efek' : 'Rantai efek dikembalikan');
    }
    ...
    createModule(id, config) {
        ...
        toggle.addEventListener('change', (e) => {
            ...
            this.maybeSwitchToCustomPreset();
            this.saveState();
        });
    ...
    createSlider(fxId, paramId, conf) {
        ...
        (this.sliders[fxId] || (this.sliders[fxId] = {}))[paramId] = { slider, updateVal };
    ...
    async handleDownload() {
        await this.triggerOverlay('Merender WAV Berkualitas Tinggi...', 'Bisa memakan waktu untuk berkas yang lebih panjang', 0);
        ...
        try {
            ...
            this.hideOverlay();
            this.showToast('Ekspor selesai');
        } catch (err) {
            console.error(err);
            this.hideOverlay();
            this.showToast('Ekspor gagal', 'error');
        }
    }
    ...
    async loadPresetFile(file) {
        try {
            ...
            this.showToast('Pengaturan dimuat');
        } catch (err) {
            console.error(err);
            this.showToast('Berkas pengaturan tidak dapat dibaca', 'error');
        }
    }
    ...
    applyPresetData(data) {
        ...
        // Audio Presets label: files saved before this feature existed
        // won't have activePreset — treat those as "Custom" (their values
        // aren't guaranteed to match any named preset exactly).
        const validPresetKeys = new Set([...PRESET_ORDER, 'custom']);
        this.state.activePreset = (typeof data.activePreset === 'string' && validPresetKeys.has(data.activePreset)) ? data.activePreset : 'custom';
        this.state.customSnapshot = data.customSnapshot || null;
        if (this.dom.audioPresetSelector) this.dom.audioPresetSelector.value = this.state.activePreset;
